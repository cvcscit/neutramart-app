...
export default function ImageUpload({ onScanComplete }: Props) {
  ...
  /**
   * Map whatever `useImageAnalysis` returns into the `SingleAnalysis[]` shape.
   *
   * `analysis` can be:
   *  - an array of ImageAnalysis objects (multi-image response from the hook)
   *  - a single ImageAnalysis object (legacy / single-image response)
   *
   * Each ImageAnalysis already has: imageKey, imageName, imageUrl, description,
   * confidence, allergens, objects, dishes, totalNutrition — exactly what
   * NutritionAnalysis expects.  We just attach the preview URL as a fallback
   * in case imageUrl isn't populated by the hook yet.
   */
  const buildAnalyses = () => {
    if (!analysis) return [];
    ...
    if (Array.isArray(raw)) {
      return raw.map((a: any, i: number) => ({
        ...a,
        imageKey:  a.imageKey  ?? `img-${i}`,
        imageUrl:  a.imageUrl  ?? previews[i],
        imageName: a.imageName ?? files[i]?.name,
      }));
    }
   
    // ── 2. Wrapped array ────────────────────────────────────────────────────
    const wrapped: any[] | undefined =
    ...
    if (wrapped) {
      return wrapped.map((a: any, i: number) => ({
        ...a,
        imageKey:  a.imageKey  ?? `img-${i}`,
        imageUrl:  a.imageUrl  ?? previews[i],
        imageName: a.imageName ?? files[i]?.name,
    ...
    // ── 3. Flat combined object ──────────────────────────────────────────────
    // The API analysed all images together and returned one flat object.
    // We create one SingleAnalysis entry per uploaded preview so each image
    // gets its own collapsible card with a thumbnail, all sharing the same data.
    return previews.map((preview, i) => ({
      ...
      objects:        raw.objects        ?? [],
      dishes:         raw.dishes         ?? [],
      totalNutrition: raw.totalNutrition ?? undefined,
      // Raw string fields — NutritionAnalysis will parse the totals from these
  ...
  return (
    <div className="my-15 container m-auto">
      ...
      {/* Hide upload card, show results card on success */}
      {status === "success" && analysis ? (
        <div>
          <div className="mb-5 flex w-full max-w-4xl mx-auto z-10 relative">
          ...
          <Card className="w-full max-w-4xl mx-auto z-10 relative">
            ...
            <CardContent className="space-y-6">
              <NutritionAnalysis analyses={buildAnalyses()} />
            </CardContent>
          </Card>
        </div>
      ) : (
