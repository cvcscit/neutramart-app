# Neutramart Backend — Production Deployment Guide

This guide walks you through deploying the Neutramart backend to AWS Fargate with production-grade security.
...
| **ALB URL** | `http://neutramart-alb-396784627.ap-south-1.elb.amazonaws.com` |
| **Health Check** | `http://neutramart-alb-396784627.ap-south-1.elb.amazonaws.com/health` |
| **S3 Bucket** | `sci-neutrasmart-project` |
| **ECR Repo** | `209479309679.dkr.ecr.ap-south-1.amazonaws.com/neutramart-backend` |
| **ECS Cluster** | `neutramart-cluster` |
| **AWS Profile** | `neutramart` (configured in `~/.aws/credentials`) |
...
| **ALB as Single Entry Point** | All traffic enters through ALB (`neutramart-alb-396784627.ap-south-1.elb.amazonaws.com`) in public subnets |
...
| **IAM Task Role** | Container uses `neutramart-ecs-task-role` — boto3 auto-discovers credentials from ECS task metadata |
| **IAM Task Execution Role** | `neutramart-ecs-execution-role` — only has permissions to pull ECR images, write CloudWatch logs, and read SSM parameters |
...
| **CloudWatch Logs** | 30-day retention in `/ecs/neutramart-backend` log group |
...
══════════════════════════════════════════════
  ...
  ALB URL:  http://neutramart-alb-xxxxxxxx.ap-south-1.elb.amazonaws.com
  Health:   http://neutramart-alb-xxxxxxxx.ap-south-1.elb.amazonaws.com/health
```
...
══════════════════════════════════════════════
  ...
  Image:  123456789.dkr.ecr.ap-south-1.amazonaws.com/neutramart-backend:abc1234
  URL:    http://neutramart-alb-xxxxxxxx.ap-south-1.elb.amazonaws.com
  Health: http://neutramart-alb-xxxxxxxx.ap-south-1.elb.amazonaws.com/health
```
...
```bash
curl http://neutramart-alb-396784627.ap-south-1.elb.amazonaws.com/health
...
In `neutramart_ui/src/config/api.js`, change the API_URL:
```javascript
...
```
VITE_API_URL=http://neutramart-alb-396784627.ap-south-1.elb.amazonaws.com/api
```
...
```bash
# Tail live logs
aws logs tail /ecs/neutramart-backend --region ap-south-1 --profile neutramart --follow

# Last 30 minutes
aws logs tail /ecs/neutramart-backend --region ap-south-1 --profile neutramart --since 30m
```
...
aws ecs describe-services \
  --cluster neutramart-cluster \
  --services neutramart-backend-service \
  --region ap-south-1 \
...
- Example: `neutramart.com` or `api.neutramart.com`
- A domain is like your address on the internet — instead of users going to `http://neutramart-alb-xxxxxxxx.elb.amazonaws.com`, they go to `https://api.neutramart.com`

### 2. Get an SSL Certificate (Free from AWS)
```bash
# Request a certificate
aws acm request-certificate \
  --domain-name api.neutramart.com \
...
- Change `API_URL` to `https://api.neutramart.com`
...
- Scale down to 0 tasks when not testing: `aws ecs update-service --cluster neutramart-cluster --service neutramart-backend-service --desired-count 0 --region ap-south-1`
- Scale back up: `aws ecs update-service --cluster neutramart-cluster --service neutramart-backend-service --desired-count 1 --region ap-south-1`
...
```bash
# Check ECS events
aws ecs describe-services --cluster neutramart-cluster --services neutramart-backend-service --region ap-south-1 --query 'services[0].events[:5]'

# Check logs
aws logs tail /ecs/neutramart-backend --region ap-south-1 --since 10m
```
...
- Check: `aws iam get-role-policy --role-name neutramart-ecs-task-role --policy-name neutramart-s3-bedrock`
